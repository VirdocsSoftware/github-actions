class ReleaseNotesGenerator {
    ...
    fromTemplate(data) {
        const createRow = ({ ticket_id, summary, impact_summary, additional_resources, stakeholder_highlight }) => {
            return `<tr><td style="text-align: center;"><a href="https://virdocs.atlassian.net/browse/${ticket_id}">${ticket_id}</a></td><td style="text-align: center;">${summary}</td><td style="text-align: center;">${impact_summary ?? ""}</td><td style="text-align: center;">${additional_resources ? `<a href="${additional_resources}">${additional_resources}</a>` : ""}</td><td style="text-align: center;">${stakeholder_highlight ? "✅" : ""}</td></tr>`;
        };
